...
Sebelum menggunakan CodeIgniter 4, konfigurasi PHP pada XAMPP dilakukan dengan mengaktifkan ekstensi berikut pada `php.ini`:
- `php-json`
- `php-mysqlnd`
- `php-xml`
- `php-intl`
...
1. Download CodeIgniter 4 dari situs resmi.
2. Extract ke folder `htdocs/lab11_ci`.
3. Rename folder menjadi `ci4`.
4. Akses pada browser: `http://localhost/lab11_ci/ci4/public`


### Menjalankan CLI CodeIgniter

Masuk ke folder project lalu jalankan perintah:
```bash
php spark
```
...
Update controller untuk memanggil view:
```php
public function about()
{
    return view('about', [
...
Pada praktikum ini telah dipelajari instalasi CodeIgniter 4, konfigurasi environment, struktur direktori CI4, konsep MVC, routing, controller, serta view dan template layout.
...
Aktifkan Apache & MySQL pada XAMPP, lalu buat database `lab_ci4` dan tabel `artikel`.
...
Pada praktikum ini berhasil dibuat sistem CRUD artikel lengkap dengan halaman admin menggunakan CodeIgniter 4 dan arsitektur MVC.
...
### Tujuan
Memahami dan mengimplementasikan View Layout dan View Cell pada CodeIgniter 4.

### View Layout

Layout digunakan untuk tampilan halaman yang konsisten. Buat `app/Views/layout/main.php` yang menggunakan:
```php
<?= $this->renderSection('content') ?>
```

Modifikasi view untuk menggunakan layout:
```php
...
View Cell adalah komponen reusable yang memiliki logika sendiri. Buat `app/Cells/ArtikelTerkini.php`:
```php
...
|---|---|
| Hanya menampilkan data | Bisa punya logika sendiri |
| Bergantung pada controller | Bisa mengambil data sendiri |

### Kesimpulan

View Layout memastikan konsistensi tampilan dan mempermudah maintenance. View Cell memungkinkan pembuatan komponen yang reusable dengan logika mandiri.
...
### Tujuan
Membuat sistem autentikasi login dengan proteksi halaman admin.
...
Fungsi utama controller login:
- Menerima input username & password.
- Memvalidasi kredensial terhadap database.
...
Pada praktikum ini berhasil dibuat sistem login dengan autentikasi sesi, proteksi halaman admin menggunakan filter, serta fitur logout.
